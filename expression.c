...
char *resolve_current_pc_and_store( char *txt ) {
    int a, b;
    char *p, *q;
    for ( a = b = 0; txt[ b ]; b++ )
        a += txt[ b ] == CURRENT_PC;
...
static void swap_internal_data( struct info **new_stack, struct info **new_list, unsigned *new_stack_top, unsigned *new_list_top ) {
    unsigned temp;
    struct info *scra;
    temp = stack_top;
...
*/
int parse_expr( char *txt, int *value, unsigned lineno ) {
    ...
    if ( !undef ) {
        ...
        for ( a = 0; a < list_top; a++ ) {
            ...
            case '!':
                ...
                break;
            case 'a':
                if ( (unsigned)right >= sizeof( left ) * 8 )
                    resu = ( left >= 0 ? 0 : -1 );
                else
                    resu = left >> right;
                break;
