...
unsigned char write_to_memory( unsigned short index, unsigned char a ) {
...
static int take_line( char *line, int max_chars ) {
    if ( !fgets( line, max_chars - 1, input ) ) {
        line[ 0 ] = 0;
        return 1;
    } /* end of file */
#ifdef UNIX
...
void error( int l, char *line, char *txt ) { fprintf( stderr, "%s\nline %d: %s\n", line, l, txt ); }


/* return value: 0=OK, 1=syntax error */
/* parses command line options */

static int parse_arg( char *arg, char **filename ) {
    unsigned long a;
    char *p, *q, *e;
...
static int compare( const struct info *left, const struct info *right ) { return strcmp( left->label, right->label ); }
...
int main( int argc, char **argv ) {
    ...
    for ( b = s = 1; s < argc && *argv[ s ] == '-'; b++ ) {
        if ( !*( argv[ s ] + b ) )
            ...
            s++;
            break;
        } /* in case filename equals option */
        else
    ...
    if ( !a ) {
        if ( fseek( input, 0, SEEK_SET ) ) {
            asm_close();
            fprintf( stderr, "can't rewind input file \"%s\".\n", argv[ s ] );
            return 2;
        }
        set_compile_pass( 2 );
        set_start_address( 0 );
    ...
    if ( !a ) {
        for ( b = 1 + s; b < argc; b++ ) {
            txt = 0;
            if ( argc == 2 + s && strlen( argv[ s ] ) >= 5 && !strcmp( argv[ s ] + strlen( argv[ s ] ) - 4, ".asm" ) ) {
                txt = argv[ s ];
    ...
    if ( !a && cross && ( b = table_entries() ) ) {
        ...
        for ( a = 0; a < b; a++ )
            printf( "%12.12s %5d %4hx %5u\n", ele[ a ].label, ele[ a ].value, (unsigned short)ele[ a ].value, ele[ a ].lineno );
        free( ele );
        a = 0;
    }
    asm_close();
