...
enum { DEFAULT, TOKEN, ADDR, HOT, GARISH, WARN, MEMORY, STATUS, HIDDEN };
static unsigned char color[] = { WHITE, GREEN, YELLOW, RED, BRIGHT, YELLOW, BLUE, PURPLE, GRAY };
...
static char __printable( char c ) { return ( c >= 32 && c < 127 ) ? c : '.'; }
    ...
    if ( row >= 0 ) {
        ...
        if ( MODE & 8 ) /* second pass disassembling */
        {
            ...
            if ( next_label_index >= labels || ele[ next_label_index ].value > current_pc )
                ...
                next_label_index += 1;
                while ( next_label_index < labels && ele[ next_label_index ].value == ele[ next_label_index - 1 ].value )
                    ele[ next_label_index ].label[ 0 ] = 0;
            }
        }
...
static void __print_r16( int x, int y, char *t, int R ) {
    char txt[ 256 ];
    c_goto( x, y );
    sprintf( txt, "%s=", t );
...
static void __print_rr( int x, int y, char *t, int R ) {
    char txt[ 256 ];
    c_goto( x, y );
    sprintf( txt, "%s=", t );
    c_setcolor( color[ TOKEN ] );
...
static void tobin( char *s, _uchar x ) {
    int a;
    for ( a = 0; a < 8; a++ )
        s[ 7 - a ] = ( ( x & ( 1 << a ) ) >> a ) ? '1' : '0';
    s[ 8 ] = 0;
...
static void __print_x( int x, int y, char *t, _uchar R ) {
    char txt[ 256 ];
    char b[ 256 ];
    tobin( b, R );
    c_goto( x, y );
...
static void __print_r( int x, int y, char *t, _uchar R ) {
    char txt[ 256 ];
    char b[ 256 ];
    tobin( b, R );
    c_goto( x, y );
...
static void print_generell_regs( void ) {
...
static void print_index_regs( void ) {
    __print_r16( 56, 7, "IX", IX );
    __print_r16( 56, 8, "IY", IY );
}


static void print_sp_and_pc( void ) {
    __print_r16( 56, 9, "SP", SP );
    __print_r16( 56, 10, "PC", PC );
}


static void print_special_regs( void ) {
    __print_x( 47, 21, "I", I );
    __print_x( 64, 21, "R", R );
}
...
static void clear_user_regs( void ) {
    A = 0;
    B = 0;
    C = 0;
    D = 0;
...
static int ask_str( char *pointer, char *message, int max_len ) {
    unsigned char c;
...
static unsigned ask( char *str, unsigned init_val ) {
    static char txt[ 256 ];

    if ( init_val )
        sprintf( txt, "%u", init_val );
    else
        txt[ 0 ] = 0;
    if ( ask_str( txt, str, 16 ) )
        return ~0;
...
/* reads a non negative integer from input */
/* on escape or error returns ~0 */
static unsigned ask_x( char *str, unsigned init_val ) {
    static char txt[ 256 ];

    if ( init_val )
        sprintf( txt, "0x%x", init_val );
    else
        txt[ 0 ] = 0;
    if ( ask_str( txt, str, 16 ) )
        return ~0;
...
static void ask_general_16_register( char *prompt, _uchar *high, _uchar *low ) {
    unsigned x;
...
static void ask_special_16_register( char *prompt, _ushort *reg16 ) {
    unsigned x;
    x = ask( prompt, *reg16 );
    if ( x > MAX_ADDR ) {
        c_bell();
...
static void ask_16bit_register( void ) {
    unsigned char c;
    c_clear( 0, 23, 79, 23 );
    do {
        c_goto( 0, 23 );
        ...
        switch ( c ) {
        case 'b':
            ask_general_16_register( "BC=", &B, &C );
            break;

        case 'B':
            ask_general_16_register( "BC'=", &B_, &C_ );
            break;

        case 'd':
            ask_general_16_register( "DE=", &D, &E );
            break;

        case 'D':
            ask_general_16_register( "DE'=", &D_, &E_ );
            break;

        case 'h':
            ask_general_16_register( "HL=", &H, &L );
            break;

        case 'H':
            ask_general_16_register( "HL'=", &H_, &L_ );
            break;
...
static void ask_8bit_register( char *prompt, _uchar *reg8 ) {
...
static void ask_register( void ) {
    ...
    do {
        ...
        case 'a':
            ask_8bit_register( "A=", &A );
            break;

        case 'A':
            ask_8bit_register( "A'=", &A_ );
            break;

        case 'f':
            ask_8bit_register( "F=", &F );
            print_flags();
            break;

        case 'F':
            ask_8bit_register( "F'=", &F_ );
            print_flags();
            break;

        case 'b':
            ask_8bit_register( "B=", &B );
            break;

        case 'B':
            ask_8bit_register( "B'=", &B_ );
            break;

        case 'c':
            ask_8bit_register( "C=", &C );
            break;

        case 'C':
            ask_8bit_register( "C'=", &C_ );
            break;

        case 'd':
            ask_8bit_register( "D=", &D );
            break;

        case 'D':
            ask_8bit_register( "D'=", &D_ );
            break;

        case 'e':
            ask_8bit_register( "E=", &E );
            break;

        case 'E':
            ask_8bit_register( "E'=", &E_ );
            break;

        case 'h':
            ask_8bit_register( "H=", &H );
            break;

        case 'H':
            ask_8bit_register( "H'=", &H_ );
            break;

        case 'l':
            ask_8bit_register( "L=", &L );
            break;

        case 'L':
            ask_8bit_register( "L'=", &L_ );
            break;

        case 'I':
            ask_8bit_register( "I=", &I );
            break;

        case 'R':
            ask_8bit_register( "R=", &R );
            break;
...
static void protocol( void ) {
    ...
    current_pc = PC;
    decode( 0, 0 );
    fprintf( stream, "%c%c%c%c ", ( is_flag( F_C ) ? 'C' : '.' ), ( is_flag( F_Z ) ? 'Z' : '.' ), ( is_flag( F_M ) ? '-' : '+' ),
             ( is_flag( F_PE ) ? 'e' : 'o' ) );
...
int main( int argc, char **argv ) {
    ...
    while ( 1 ) {
        ...
        if ( cpu_pin[ busrq ] )
            ...
            if ( cpu_is_in_disassemble && c != no_key_code )
                set_cpu_pin( halt, 0 );
        }
        if ( c != no_key_code )
            switch ( c ) {
            ...
            case 'R':
                if ( !cpu_pin[ halt ] || c == 'R' )
                    cpu_is_in_disassemble = !cpu_is_in_disassemble;
                if ( !cpu_is_in_disassemble && !speed )
                    speed = 1;
                print_panel();
                break;
            ...
            case '.': /* enter instruction */
                ...
                do {
                    ...
                    if ( ask_str( string, "Put instruction into memory: ", 40 ) )
                        break;
                    compile( string + 7 );
            ...
            case 'U':
                ...
                if ( ask_str( string + 13, string, 100 ) )
                    break;
            ...
            case 'W':
                ...
                if ( ask_str( string + 12, string, 64 ) )
                    break;
                set_start_address( MEMP );
                compile( string + 7 );
                print_panel();
            ...
            case 'L': /* load */
                if ( ask_str( load_name, "Load file: ", 40 ) ) {
                    c_bell();
                    print_status();
                    break;
                }
                ...
                if ( !stream ) {
                    stream = 0;
                    error_msg( load_name, "Can't read file" );
                    print_status();
                    break;
                }
                if ( read_header( stream, &start, &x ) ) {
                    warning( "Not a Z80 ASM file" );
                    start = ask( "Load to address: ", PC );
                } else {
                    sprintf( string, "starting at 0x%x  %u bytes put", a, x );
                    warning( string );
                }
                dma_write( start, x, stream );
                fclose( stream );
            ...
            case 'S': /* save */
                if ( ask_str( save_name, "Save as: ", 40 ) ) {
                    c_bell();
                    break;
                }
                x = ask( "Length: ", 0 );
                ...
                if ( !stream ) {
                    stream = 0;
                    error_msg( save_name, "Can't write to file " );
                    print_status();
                    break;
                }
            ...
            case 'D': /* dump/disassemble */
                if ( ask_str( save_name, "Disassemble to: ", 40 ) ) {
                    c_bell();
                    break;
                }
                ...
                if ( !stream ) {
                    stream = 0;
                    error_msg( save_name, "Can't append to file" );
                    print_status();
                    break;
                }
                row = -2;
            ...
            case '\"': /* protocol execution */
                if ( ask_str( proto_name, "Protocol into: ", 40 ) ) {
                    c_bell();
                    break;
                }
                ...
                if ( !stream ) {
                    stream = 0;
                    error_msg( proto_name, "Can't append to file" );
                    print_status();
                    break;
                }
        ...
        if ( cpu_is_in_disassemble )
            emu = 0; /* really finish emulation mode ?? */
        if ( IFF0 )
            IFF2 = IFF1 = 1;
