...
unsigned table_entries( void ) { return counter; }


/* hashing function */

static unsigned hashl( char *slovo, unsigned len ) {
    unsigned b = 0;
...
static unsigned hash( char *slovo ) { return hashl( slovo, strlen( slovo ) ); }
...
int add_to_table( char *key, int value, unsigned lineno, bool copy ) {
    unsigned a, c;
    a = hash( key );
    c = entry[ a ].count;
    if ( !( c & c - 1 ) ) {
...
bool reaccess_label( char *key, unsigned lineno ) {
    int a, b;
    ...
    for ( b = 0; b < entry[ a ].count; b++ )
        if ( !strcmp( key, entry[ a ].element[ b ].pointer ) && ( !lineno || entry[ a ].element[ b ].lineno == lineno ) ) {
...
int is_in_table( char *key, unsigned len, int *value, unsigned lineno ) {
    int a, b, c;
    ...
    for ( b = 0; b < entry[ a ].count; b++ )
        if ( len && !strncmp( key, entry[ a ].element[ b ].pointer, len ) ||
            ...
            if ( !entry[ a ].element[ b ].uniq && lineno && entry[ a ].element[ b ].lineno > lineno )
                break;
            c = b;
...
bool last_label_reusable( void ) { return !just_ele->uniq; }
