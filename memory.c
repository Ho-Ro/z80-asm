...
#ifdef NEEDED
static int set_bank_port( unsigned char port_id, unsigned char value ) {
#ifdef OLD_STYLE
    FILE *fp;
...
static unsigned char *update_all_adress_maps( unsigned char bank_id ) {
    unsigned i, j, t = 0;
    unsigned char id, end = 0; /* to prevent compiler warning uninitialization */
    bool first = 1;
    for ( i = 1 << 16 - delta; i--; ) {
        bool update = 0;
        if ( address_map[ i ] == memory )
            continue;
        id = mapped_from[ i ];
...
static void bank_switch( unsigned i, unsigned char id ) {
    if ( map[ i ].virtuel ) {
...
void set_default_byte( _uchar val ) { empty = val; }


void switch_bank( _uchar val, unsigned map_no ) {
    unsigned i, j;
...
unsigned bank_port_index( unsigned char id ) {
    unsigned i;
    for ( i = 0; i < no_maps; i++ )
        if ( map[ i ].port_no == id )
            return i + 1;
...
int init_banks( char *rom_path, char *bank_mapping_descr ) {
    ...
    for ( k = 1; fgets( buffer, 800, fp ); k++ ) {
        ...
        } else /* page description line */
        {
            ...
            struct port_map *mmm;
            for ( i = 0; i < 5; i++ )
                if ( tok1[ i ] >= '0' && tok1[ i ] <= '9' )
                    continue;
                else if ( tok1[ i ] >= 'a' && tok1[ i ] <= 'f' )
                    continue;
                else if ( tok1[ i ] >= 'A' && tok1[ i ] <= 'F' )
                    continue;
                else
            ...
            for ( j = 0; j < 2; j++ )
                if ( tok3[ j ] >= '0' && tok3[ j ] <= '9' )
                    continue;
                else if ( tok3[ j ] >= 'a' && tok3[ j ] <= 'f' )
                    continue;
                else if ( tok3[ j ] >= 'A' && tok3[ j ] <= 'F' )
                    continue;
                else
...
_uchar memory_at( unsigned short index ) {
    return !no_maps                                                             ? memory[ index ]
           : !all_readable && ( access_type >> ( 2 * ( index >> delta ) ) & 1 ) ? empty
                                                                                : *( address_map[ index >> delta ] + index );
}
...
unsigned dma_write( unsigned short offset, unsigned count, FILE *from ) {
    return fread( memory + offset, 1, count + offset < (unsigned)MAX_MEM ? count : (unsigned)MAX_MEM - offset, from );
}


unsigned dma_read( unsigned short offset, unsigned count, FILE *to ) {
    return fwrite( memory + offset, 1, count + offset < (unsigned)MAX_MEM ? count : (unsigned)MAX_MEM - offset, to );
}
