...
int c_adc_sbc( int first, int type, int second, int offset, int d ) {
    if ( first == R_A ) {
        if ( type == A_PODLE_REG && second == R_HL ) {
            out( offset + 6 );
            return 0;
        }
        if ( type == A_NUM ) {
...
int c_add( int first, int type, int second ) {
    if ( first == R_A ) {
        if ( type == A_PODLE_REG && second == R_HL ) {
            out( 0x86 );
            return 0;
...
int c_logical( int type, int value, int offset ) {
    switch ( type ) {
    case A_REG:
        switch ( value ) {
        case R_A:
...
int c_inc_dec( int type, int value, int offs1, int offs2 ) {
    switch ( type ) {
    case A_REG:
        switch ( value ) {
        case R_A:
...
int c_out( int t1, int v1, int v2 ) {
...
int c_push_pop( int v, int offset ) {
    switch ( v ) {
    case R_AF:
        out( 0xf0 + offset );
        break;
...
int c_bit( int b, int type, int value, int offset ) {
    switch ( type ) {
    case A_REG:
        switch ( value ) {
        case R_A:
...
int c_shift_rot( int type, int value, int offset ) {
    switch ( type ) {
    case A_REG:
        switch ( value ) {
        case R_A:
...
int c_jp( int t1, int v1, int t2, int v2 ) {
    if ( t2 == A_EMPTY )
        switch ( t1 ) {
        case A_REG:
        case A_PODLE_REG:
...
int c_call( int t1, int v1, int t2, int v2 ) {
    if ( t2 == A_EMPTY )
        switch ( t1 ) {
        case A_NUM:
            out( 0xcd );
...
int c_jr( int t1, int v1, int t2, int v2 ) {
    int a, c;
...
int c_ld( int t1, int v1, int t2, int v2 ) {
    int a = 0, b = 0;
    switch ( t1 ) {
