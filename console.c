...
/* sets foreground color */
{
    ...
    if ( a > 7 )
        printf( "\033[1;%dm", 30 + a - 8 );
    else
        printf( "\033[0;%dm", 30 + a );
#else
    textcolor( a );
#endif
}
...
unsigned char c_getkey( void ) {
    int k;
#ifdef UNIX
    while ( ( k = getchar() ) == EOF )
        ; /* getchar may not block in non canonical mode */
    switch ( k ) {
    case 9:
        return K_TAB;
...
#else
    while ( ( k = getch() ) == EOF )
        ; /* getch may not block in non canonical mode */
    switch ( k ) {
    case 9:
        return K_TAB;
