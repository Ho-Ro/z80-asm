...
static int todd( _uchar *t1, _uchar *a1, _uchar *t2, _uchar *a2 ) {
    if ( *t1 == A_REG && *a1 == R_HL )
        *a1 = R_IX;
    if ( *t2 == A_REG && *a2 == R_HL )
        *a2 = R_IX;
    ...
    else if ( *t2 == A_PODLE_REG && *a2 == R_HL )
...
static int tofd( _uchar *t1, _uchar *a1, _uchar *t2, _uchar *a2 ) {
    if ( *t1 == A_REG && *a1 == R_HL )
        *a1 = R_IY;
    if ( *t2 == A_REG && *a2 == R_HL )
        *a2 = R_IY;
...
void decode( _ushort *new_pc, int exec ) {
    ...
    normal:
        ...
        if ( ddfd_prefix && p->func != f_exx && ( p->func != f_ex || p->type1 != A_REG ) ) {
            if ( iy )
                join = tofd( &current_instr.type1, &current_instr.arg1, &current_instr.type2, &current_instr.arg2 );
            else
                join = todd( &current_instr.type1, &current_instr.arg1, &current_instr.type2, &current_instr.arg2 );
            if ( current_instr.type1 == A_PODLE_IY_PLUS || current_instr.type1 == A_PODLE_IX_PLUS ) {
                current_instr.arg1 = read_opcode( PC, 0 );
                PC++;
            }
            if ( current_instr.type2 == A_PODLE_IY_PLUS || current_instr.type2 == A_PODLE_IX_PLUS ) {
                current_instr.arg2 = read_opcode( PC, 0 );
                PC++;
            }
        }
        break;
